...
public class CreateWorkshopTokenFeatureTests : IClassFixture<IntegrationTestWebAppFactory>
{
    ...
    public CreateWorkshopTokenFeatureTests(IntegrationTestWebAppFactory factory)
    {
    ...
    public async Task LoginWorkshop_Should_ReturnOkAndToken_WhenCredentialsAreValid()
    {
        ...
        var loginResponse = await response.Content.ReadFromJsonAsync<CreateWorkshopTokenResponse>();
